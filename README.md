...
The split files are static — host them anywhere that serves static files. With
GitHub Pages:

1. Push this folder to a GitHub repo.
2. Settings → Pages → Source: deploy from a branch → `main` / `(root)`.
3. Lives at `https://<user>.github.io/<repo>/`, with free HTTPS and optional
   custom domain.
...
the licence is non-commercial, so this tool is free; charging for it would
require a separate commercial licence from Jagex.
...
The original source code of this project (HTML, CSS, JavaScript) is released
under the MIT license — see `LICENSE`. This covers only the code authored here;
third-party content (Old School RuneScape names/IP under Jagex's Fan Content
Policy, and the Lucide icons under the ISC license) is covered separately in
`NOTICE.md`. If you fork or reuse the code, keep `NOTICE.md` intact.
